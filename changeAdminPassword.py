...
class Main:
    def __init__(self, adminID):
        ...
        self.mainLabel = Label(self.root, text="Change Admin Password", font=('arial', 20, 'bold'))
        ...
        self.lb1 = Label(self.f, text="Your Admin ID", font=self.font,bg=self.lb0color)
        self.txt1 = Entry(self.f, width=25, font=self.font,relief=SOLID)
        self.lb1.grid(row=0, column=0, padx=10, pady=10)
        self.txt1.grid(row=0, column=1, padx=10, pady=10)
        self.txt1.insert(0, adminID)
    ...
    def updatePass(self):
        ...
        cr = conn.cursor()
        q = f"SELECT * from admin where id='{id}' and password='{oldpass}'"
        cr.execute(q)
        data = cr.fetchall()
        if len(data) == 0:
            msg.showwarning('Warning', "Incorrect Old Password", parent=self.root)
        else:
            q1 = f"update admin set password='{newpass}' where id='{id}'"
            cr.execute(q1)
            conn.commit()
