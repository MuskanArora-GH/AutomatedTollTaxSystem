...
class Main:
    ...
    def updateTollPrice(self):
        ...
        if bool(re.match('^[0-9]+$', price)):
            q = f"update monthlypass set category='{category}', price='{price}',description='{description}' where id='{id}'"
            self.cr.execute(q)
            self.conn.commit()
    ...
    def getValues(self):
        ...
        q = 'select id,category,price,description from monthlypass'
        self.cr.execute(q)
    ...
    def deleteCategory(self):
        ...
        data = item['values']
        categoryID = data[0]
        q = f'delete from monthlypass where id="{categoryID}"'
        self.cr.execute(q)
        self.conn.commit()
