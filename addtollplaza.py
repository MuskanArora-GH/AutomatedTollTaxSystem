...
class Main:
    def __init__(self):
        ...
        self.mainLabel = Label(self.root, text="Add Toll Plaza", font=('arial', 20, 'bold'),bg=self.maincolor,borderwidth=4,relief=SUNKEN)
        ...
        self.btn = Button(self.root, text="Submit", font=self.font, command=self.insertToll,bg=self.btncolor,activebackground=self.activebutton)
    ...
    def insertToll(self):
        ...
        if len(name) == 0 or len(email) == 0 or len(mobile) == 0 or len(password) == 0 :
            ...
            if connection.verify(email) == 'Invalid' or connection.check(mobile) == 'Invalid':
                ...
                q = f"insert into toll values (null,'{name}', '{email}','{mobile}', '{password}')"
                cr.execute(q)
                conn.commit()
